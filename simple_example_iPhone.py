...
data = np.loadtxt('wikipedia-20150701-20161231-iphone.txt')
...
n_outputs = 1
n_reservoir = 200  # number of recurrent units
...
train_in, test_in, train_out, test_out = train_test_split(np.arange(1, data.size + 1), data, test_size=.2)

# print(train_test_split(np.arange(data.size), data, test_size=.2))

# train_in = data[None, 0:trainLen]
# train_out = data[None, 0 + 1:trainLen + 1]
# test_in = data[None, trainLen:trainLen + testLen]
# test_out = data[None, trainLen:trainLen + testLen]
...
# print( 'MSE = ' + str( mse ))
mse = np.mean(
    (test_out[:] - output_pred[0]) ** 2)  # Mean Squared Error: see https://en.wikipedia.org/wiki/Mean_squared_error
rmse = np.sqrt(
    mse)  # Root Mean Squared Error: see https://en.wikipedia.org/wiki/Root-mean-square_deviation for more info
